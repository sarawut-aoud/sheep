...
class Imap
{
	 ...
	 * @param array $config
	 */
	public function __construct(array $config = [], $messageNumber = NULL)
	{
	...
	public function listMessages($page = 1, $per_page = 25, $sort = null, $sort_unseen = null)
	{
		$limit = ($per_page * $page);
		...
		if ($sort_unseen != null) {
			...
			$sorted = imap_sort($this->stream, $by, $direction, 0, 'UNSEEN');
		...
		if (true === is_array($sorted)) {
			...
			foreach ($msgs as $msgno) {
				...
				$attach[$msgno][] = $this->get_att_file($msgno);
			}
		}
		$return = array(
			'res' => $result,
			...
			'unseen' => count(imap_search($this->stream, 'UNSEEN')),
			// 'seen' => imap_setflag_full($this->stream, $email_number, "\\Seen"),
		);
		$return['pages'] = ceil($return['total'] / $per_page);

		// $return['attachfile'] = (array); 


		return $return;
	}

	public function listMessagesSearch($msgs = null)
	{
	...
	public function listMessagesSortUnseen($page = 1, $per_page = 25, $sort = null)
	{
		$limit = ($per_page * $page);
		$start = ($limit - $per_page) + 1;
		$start = ($start < 1) ? 1 : $start;
	...
	private function get_att_file($message_number)
	{
		$structure = imap_fetchstructure($this->stream, $message_number);
		$attachments = array();
		if (isset($structure->parts) && count($structure->parts)) {
			...
			for ($i = 0; $i < count($structure->parts); $i++) {
				...
				// 	$attachments[$i]['attachment'] = imap_fetchbody($this->stream, $message_number, $i+1);
			...
			if (!empty($structure->parts)) {
				...
				if ($structure->type == 0) {
					if ($structure->subtype == 'PLAIN') {
						$this->bodyPlain  = $this->getPart(1, $structure->encoding);
					} else {
						$this->bodyHTML .= $this->getPart(2, $structure->encoding);
					}
				}
			}
	...
	public function recurse($messageParts, $prefix = '', $index = 1, $fullPrefix = true)
	{
		foreach ($messageParts as $part) {
			...
			if ($part->type == 0) {
				...
				$msg = new EmailMessage($this->stream, $this->messageNumber);
	...
	public function getPart($partNumber, $encoding)
	{
		...
		$data = imap_fetchbody($this->stream, $this->messageNumber, $partNumber);
		switch ($encoding) {
			case 0:
				return $data; // 7BIT
			case 1:
				return quoted_printable_decode(imap_8bit($data)); // 8BIT
			case 2:
				return $data; // BINARY
			case 3:
	...
	public function connect_custom(array $config = [])
	{
		$config       = array_replace_recursive($this->config, $config);
		$this->config = $config;
	...
	public function setImapDraftMail($from = "", $to = "", $subject = "", $body = "", $date)
	{

		$message = "From: $from\r\n"
			. "To: " . $this->convert_to_utf8($to) . "\r\n"
			. "Subject: " . $this->convert_to_utf8($subject) . "\r\n"
			. "\r\n"
			. $this->convert_to_utf8($body) . "\r\n";

		$append = imap_append($this->stream, $this->mailbox . "Drafts", $message, "", $date);
